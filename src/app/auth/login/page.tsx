...
import { signInUser, signInWithOAuthProvider } from '@/app/actions'
  ...
  return (
    <div className="w-full max-w-sm mx-auto relative z-10 flex flex-col justify-center">
      ...
      <div className="mb-10">
        ...
        <p className="text-sm text-slate-400">Log in to your intelligence dashboard.</p>
      </div>
      ...
      <form onSubmit={handleLogin} className="space-y-4">
        ...
        <div>
          <label className="block text-xs font-medium text-slate-400 mb-1.5 flex justify-between">
            Password
            <a href="#" className="text-slate-500 hover:text-slate-300 transition-colors">Forgot?</a>
          </label>
          <input
            type="password"
            value={password}
      ...
      <p className="mt-8 text-center text-xs text-slate-500">
        Don't have an account? <Link href="/auth/signup" className="font-medium text-slate-300 hover:text-white transition-colors">Sign up</Link>
      </p>
    </div>
  )
}
